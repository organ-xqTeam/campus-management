...
public class ExcelUtil2<T>
{
    ...
    public inputxlsxresut importExcelnew(InputStream is) throws Exception
    {	
    	
    	inputxlsxresut inputxlsx = new inputxlsxresut();
    	...
    	//读表格数据
    	List<List<String>> llst =  read(wb);
    	
    	
    	int optioni = -1;
    	List<String> l2 = llst.get(0);
    	List<String> value = new ArrayList<>();
    	for (int i = 0; i < l2.size(); i++) {
			if (!"".equals(l2.get(i)) && !"-".equals(l2.get(i))) {
				value.add(l2.get(i));
				if ("选项".equals(l2.get(i))) {
					optioni = i;
				}
			}
		}
    	...
    	if (optioni != -1) {
    		for (int i = 0; i < llst.size(); i++) {
    			List<String> va = llst.get(i);
    			if (!"-".equals(va.get(optioni)) && !"选项".equals(va.get(optioni))) {
    				set.add(va.get(optioni));
				}
    		}
		}
    	inputxlsx.setSet(set);
//    	value.add(titles);
//		maps.put(ti, value);
    	inputxlsx.setList(value);
    	String rownum = "";
    	List<String[]> lo = new ArrayList<String[]>();
    	String[] sse = new String[2];
    	for (int i = 1; i < llst.size(); i++) {
    		List<String> l1 = llst.get(i);
			if ("-".equals(l1.get(0))) {
				//获取需要 ，如果是空就是用上一个序号
			}else {
				//如果当前序号 和 上一序号不同 储存 之前读取得数据
				if (!rownum.equals(l1.get(0)) && !"".equals(rownum)) {
					maps.put(sse, lo);
					sse = new String[2];
			...
			//循环后面选项得字段
			String[] ssd = new String[l1.size()];
			for (int j = 2; j < l1.size(); j++) {
				String str1 =  l1.get(j);
				ssd[j-2] = str1;
			}
			lo.add(ssd);
			//循环后面选项得字段结束 存入string 数组中
		}
    	maps.put(sse, lo);
    	
    	inputxlsx.setMaps(maps);
    	return inputxlsx;
    }
    ...
    public List<List<String>> read(Workbook wb) {
		...
		for (int i = 0; i < rowCount; i++) {
			...
			for (int c = 0; c < row.getLastCellNum(); c++) {
				...
				boolean flag = cellValue == null|| "".equals(cellValue);
//				System.out.print(flag ?  "-" :cellValue + "\t");
				rowList.add(flag ?  "-" :cellValue);
			}
//			System.out.println();
