        ...
        try {
            ...
            if (img.getWidth(null) == -1) {
                ...
                if(img.getWidth(null) <= outputWidth && img.getHeight(null) <= outputHeight && !enlarge) {
                    FileUtil.copyFileCover(inputDir + inputFileName, outputDir + outputFileName, true);
                } else {
...
//            }
    ...
    public static void pressImage(String targetImg, String waterImg, int position, float alpha, boolean sizeFixed) {
        try {
            ...
            int top;
            switch (position) {
                case Constants.WATERMARK_POSITION_TOP_LEFT:
                    left = WATERMARK_MARGIN;
                    top = WATERMARK_MARGIN;
                    break;
                case Constants.WATERMARK_POSITION_TOP_RIGHT:
                    left = width - WATERMARK_MARGIN - logoWidth;
                    top = WATERMARK_MARGIN;
                    break;
                case Constants.WATERMARK_POSITION_BOTTOM_LEFT:
                    left = WATERMARK_MARGIN;
                    top = height - WATERMARK_MARGIN - logoHeight;
                    break;
                case Constants.WATERMARK_POSITION_BOTTOM_RIGHT:
                    left = width - WATERMARK_MARGIN - logoWidth;
                    top = height - WATERMARK_MARGIN - logoHeight;
                    break;
                case Constants.WATERMARK_POSITION_CENTER:
